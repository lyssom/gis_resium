...
function formatData(data) {
    ...
    data.forEach(function (record) {
        console.log(record.header.parameterCategory + "," + record.header.parameterNumber)
    ...
    // @ts-ignore
    console.log('uComp', uComp, 'vComp', vComp);
    if (!vComp || !uComp)
        { return; }
        ...
        if (target && source && isValide(target[0]) &&
            ...
            if (pointPrev && pointNext) {
                ...
                if (isFunction(this.options.colorScale)) {
                    ...
                    var colorIdx = indexFor(particle.m, min, max, this.options.colorScale);
                    console.log(666666777)
                    console.log(colorIdx)
                    console.log(this.options.colorScale)
                    this.ctx.strokeStyle = this.options.colorScale[colorIdx];
                }
                if (isFunction(this.options.lineWidth)) {
                    // @ts-ignore
...
class CesiumWind {
  ...
  project(coordinate) {
    ...
    const sceneCoor = SceneTransforms.worldToWindowCoordinates(
      scene,
      position,
    );
...
export { CesiumWind, Field, WindLayer };
